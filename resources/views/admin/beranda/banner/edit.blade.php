...
<body>
    ...
    <main class="container mx-auto mt-5 pb-3">
        ...
        <form action="{{ route('edit_banner_aksi') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{ $banner->id }}">
            <div class="col-md-12">
                <label for="textJudul" class="form-label">Judul</label>
                <input type="text" class="form-control" id="textJudul" required name="judul" value="{{ $banner->judul }}">
            </div>
            <div class="col-md-12">
                <label for="textDeskripsi" class="form-label">Deskripsi</label>
                <input type="text" class="form-control" id="textDeskripsi" required name="deskripsi" value="{{ $banner->deskripsi }}">
            </div>
            <br>
