...
<body>
    ...
    <main class="container mx-auto mt-5 pb-3">
        <h1>Ubah Data Tujuan</h1>
        <div class="border-top mt-5 pt-5">
        <form action="{{ route('edit_tujuan_aksi') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{ $tujuan->id }}">
            <div class="col-md-12">
                <label for="textDeskripsi" class="form-label">Deskripsi</label>
                <input type="text" class="form-control" id="textDeskripsi" name="isi" value="{{ $tujuan->isi }}" required>
            </div>
            <br>
